...
if [ "$OPSC_IMAGE" != "" -a "$OPSC_IMAGE" != "none" ]; then
  ...
  docker run --link opscenter -d -e CLUSTER_NAME="$CLUSTER_NAME" -e STOMP_INTERFACE="$STOMP_INTERFACE" --name node1 -p 9042:9042 $DSE_IMAGE $NODE_OPTS
  echo "Seed node IP address for OpsCenter cluster lookup"
  docker inspect --format '{{ .NetworkSettings.IPAddress }}' node1
else
  echo "Run node node1"
  docker run -d -e CLUSTER_NAME="$CLUSTER_NAME" --name node1 -p 9042:9042 $DSE_IMAGE $NODE_OPTS
  docker exec -it node1 /etc/init.d/datastax-agent stop
  docker exec -it node1 /etc/init.d/datastax-agent start
fi
...
while [ $n != $NUM_NODES ]; do
  ...
  if [ "$OPSC_IMAGE" != "" -a "$OPSC_IMAGE" != "none" ]; then
    ...
    docker exec -it node${n} /etc/init.d/datastax-agent stop
    docker exec -it node${n} /etc/init.d/datastax-agent start
  else
    echo "Run node node${n}"
    docker run -d -e SEEDS=$SEEDS -e CLUSTER_NAME="$CLUSTER_NAME" --name node${n} $DSE_IMAGE $NODE_OPTS
  fi
done
echo "SEEDS=$SEEDS, CLUSTER_NAME=$CLUSTER_NAME DSE_IMAGE=$DSE_IMAGE, NODE_OPTS=$NODE_OPTS, STOMP_INTERFACE=$STOMP_INTERFACE" > /home/docker/git/opscenter6/cluster.info
